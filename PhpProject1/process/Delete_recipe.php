...
// delete comments
$sql = "DELETE FROM Comment WHERE recipeID = ?";
$stmt = mysqli_prepare($conn, $sql);
mysqli_stmt_bind_param($stmt, "i", $recipeID);
mysqli_stmt_execute($stmt);
 
// delete likes
$sql = "DELETE FROM Likes WHERE recipeID = ?";
$stmt = mysqli_prepare($conn, $sql);
mysqli_stmt_bind_param($stmt, "i", $recipeID);
mysqli_stmt_execute($stmt);
 
// delete from favourites
$sql = "DELETE FROM Favourites WHERE recipeID = ?";
$stmt = mysqli_prepare($conn, $sql);
mysqli_stmt_bind_param($stmt, "i", $recipeID);
mysqli_stmt_execute($stmt);
 
// delete reports
$sql = "DELETE FROM Report WHERE recipeID = ?";
$stmt = mysqli_prepare($conn, $sql);
mysqli_stmt_bind_param($stmt, "i", $recipeID);
mysqli_stmt_execute($stmt);
 
// now delete the recipe itself
$sql = "DELETE FROM Recipe WHERE id = ? AND userID = ?";
$stmt = mysqli_prepare($conn, $sql);
mysqli_stmt_bind_param($stmt, "ii", $recipeID, $userID);
mysqli_stmt_execute($stmt);
